...
$(function () {
    ...
    function add_delete_button($formset_form) {
        ...
        .each(function () {
            var $checkbox = $(this)
            var $deleteLink = $(
                '<button class="delete btn btn-sm btn-danger mb-3">-</button>'
            )
            $formset_form.find('.delete-btn-content').append($deleteLink)
            $checkbox.closest('.form-group').hide()
        });

    }

    $(document).on('click', '.add-inline-form', function (e) {
        e.preventDefault();
